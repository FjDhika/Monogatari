...
function renderCardStory($data){
	...
	if($data){
		foreach ($data as $key => $value) {
			$row[] = "<tr role='row' class='odd col-lg-3 col-md-4 col-sm-12 clickable-row' style='margin-bottom:50px;'
...
function renderCardRecom($data){
	...
	if($data){
		foreach ($data as $key => $value) {
			$row[] = "<div class='col-lg-3 col-md-4 col-sm-12 clickable-row' style='margin-bottom:50px;'
					data-href='".site_url('story/'.$value->STORY_ID)."'>
						<div class='card shadow'> 
							<img src='".base_url("assets/image/cover/$value->COVER")."' class='card-img-top'>
							<div class='card-body' style='text-align:center;'>
